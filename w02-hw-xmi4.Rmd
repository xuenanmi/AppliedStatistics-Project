...
$\hat{\beta_0}$ is the estimated mean cat's heart weight for cat's body weight is zero kg.

$\hat{\beta_1}$ is the estimated increase in mean cat's heart weight for an increase in body weight of one kg. 
...
The predicted heart weight is 12.14893 grams. Because 3.1 is within the range of cats dataset, that is interpolating, so we are confident in this prediction.
...
The predicted heart weight is 5.694432 grams. Because 1.5 is outside the range of cats dataset, that is extrapolating, so we are not very confident in this prediction.
...
```{r}
plot(Hwt ~ Bwt, data=cats, cex = 1, pch =20, col = 'blue',xlab = "Body weight (kg)", ylab = "Heart weight (g)", main = 'Heart weight vs Body weight')
abline(cat_model,col='red',lwd = 2)
```
...
The value of $R^2$ is 0.6466209, which means 64.66% of variation in the heart weight is explained by the linear relationship with body weight.
...
get_sd_est = function (fitted_vals, actual_vals, mle){
  sigma_hat = sqrt(sum((fitted_vals - actual_vals)^2)/length(fitted_vals))
  s_e = sqrt(sum((fitted_vals - actual_vals)^2)/(length(fitted_vals)-2))
  ifelse(mle==TRUE, sigma_hat, s_e)
}
```
...
When we use the cat's body weight and cat_model to predict heart weight, the standard deviation of estimated cat's heart weight is 1.452373 grams. 
...
When we use the cat's body weight and cat_model to predict heart weight, the standard deviation of estimated cat's heart weight is 1.442252 grams. 
...
df= sim_slr(x = x,beta_0 = 5, beta_1 = -3, sigma = sqrt(10.24))
```
...
```{r}
q3_model = lm (response ~ predictor, data= df)
coef(q3_model)
```

Yes, the estimated coefficients is close to the known parameters $\beta_0$ and $\beta_1$.
...
```{r}
plot(response ~ predictor, data= df, cex = 1, pch =20, col = 'grey',xlab = "predictor", ylab = "response",main = "Simulated Data")
abline(q3_model, col= 'red',lwd = 2,lty=1)
abline(5,-3, col= 'blue',lwd = 2,lty=2)
legend("topright",c("Estimated","Truth"),lty= c(1,2), col = c('red','blue'))
```
...
for (i in 1:length(beta_hat_1)){
  df_tmp= sim_slr(x = x,beta_0 = 5, beta_1 = -3, sigma = sqrt(10.24))
  model = lm (response ~ predictor, data= df_tmp)
...
The mean of beta_hat_1 is very similar to the known parameter $\beta_1$ = -3.


**(f)** Plot a histogram of `beta_hat_1`. Comment on the shape of this histogram.

```{r}
hist(beta_hat_1, col ='blue', xlab = expression(hat(beta)[1]), main = "")
```

The shape of histogram seems to follow a normal distribution with mean close to -3.
...
for (i in 1:length(beta_hat_1)){
  df_tmp= sim_slr(x = x,beta_0 = 3, beta_1 = 0, sigma = sqrt(4))
  model = lm (response ~ predictor, data= df_tmp)
  beta_hat_1[i] = coef(model)[2]
}
```
...
```{r}
hist(beta_hat_1, col ='blue',main = "", xlab = expression (hat(beta)[1]))
```

The shape of histogram seems to follow a normal distribution with mean close to 0.
...
```{r}
setwd('/Users/Xuenan/Documents/UIUC/2020Summer/Stat420/week2')
skeptic<-read.csv('skeptic.csv',h=T)
model4c<-lm(response ~ predictor, data= skeptic)
c_beta_hat_1<-coef(model4c)[2]
c_beta_hat_1
```
...
```{r}
hist(beta_hat_1, col ='blue', main = "", xlab= expression(hat(beta)[1]))
abline(v = c_beta_hat_1, col = 'red',lwd=2)
```
...
```{r}
mean(beta_hat_1 < c_beta_hat_1)
2 * mean(beta_hat_1 < c_beta_hat_1)
```
...
I think the skeptic data could be generated by the model, but the probability is very small. Becasue the predictor for skeptic data is -0.2221927, which is within the range of $\hat{\beta_1}$ values in the simulation data, so we could use the simulation data to generate skeptic data. But the -0.2221927 is a extreme value, only 0.24% of the simulation data is smaller than -0.2221927.
...
library(knitr)
# fit three models
model_q5a1 <- lm(ozone ~ wind, data=Ozone)
model_q5a2 <- lm(ozone ~ humidity, data=Ozone)
model_q5a3 <- lm(ozone ~ temp, data=Ozone)
...
model_list = list (model_q5a1, model_q5a2, model_q5a3)
q5a_df = data.frame(
  Predictor = c('wind','humidity','temp'),
  RMSE = sapply (model_list, rmse),
  R2 = sapply (model_list,r2)
)
...
The temperature predictor is the most helpful for predicting ozone readings.
Because R2 for temperature is the largest, which indicates 60.42% of the variation in the ozone can be explained by the linear relationship with temperature. And, the RMSE for temperature is the smallest, which means this model has the smallest the average of the residuals, so this model is better.
...
so we need to take the first deviation to ${\beta}$

\[
\frac{\partial f}{\partial\beta} = \sum_{i=1}^{n}{2}(y_{i}-\beta x_{i})(-x_{i}) = 2{\beta}\sum_{i=1}^{n}x_{i}^{2} - 2\sum_{i=1}^{n}x_{i}y_{i} = 0
\]
...
```{r}
get_beta_hat( x= cats$Bwt, y= cats$Hwt)
```

The estimate for $\beta$ is 3.907113
